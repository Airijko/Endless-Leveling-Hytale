...
/**
 * Encapsulates Arcane Dominance tuning (window/cooldown/percent).
 */
public record ArcaneDominanceSettings(boolean enabled,
    ...
    public static ArcaneDominanceSettings fromSnapshot(ArchetypePassiveSnapshot snapshot) {
        if (snapshot == null) {
            return disabled();
        }
        double reflectPercent = Math.max(0.0D, snapshot.getValue(ArchetypePassiveType.ARCANE_DOMINANCE));
        ...
        List<RacePassiveDefinition> definitions = snapshot.getDefinitions(ArchetypePassiveType.ARCANE_DOMINANCE);
        ...
        return new ArcaneDominanceSettings(true,
                reflectPercent,
                window,
                cooldown,
                targetSlowPercent,
    ...
    public static ArcaneDominanceSettings disabled() {
        return new ArcaneDominanceSettings(false, 0.0D, DEFAULT_WINDOW, DEFAULT_COOLDOWN, 0.0D, 0.0D);
    }
